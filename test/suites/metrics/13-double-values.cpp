 ...
 *
 * 13-double-values.cpp - cojson tests, code size metrics
 * metric=read/write two double values
...
static double& double2() noexcept {
	static double val;
	return val;
}
...
static void run(lexer& in, ostream& out) {
	...
	V<double, double2>().read(in);
	V<double, double2>().write(out);
}
