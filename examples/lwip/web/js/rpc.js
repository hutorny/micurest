...
function PutObj(url, obj, type, fn) {
	Send('PUT', url, JSON.stringify(obj) + '\r\n', type||'application/json', function(err, data, ctype){
		if( data && ctype ==  'application/json' ) 
		  try { 
			  data = JSON.parse(data);		
		  } catch(e) {
...
function Init() {
	...
	if( right ) {
		right.innerHTML += '<label>About...</label>' +
			'<div><a target="_new" href="http://r.iot-ware.com/r/about.html">This example</a></div>' +
			'<div><a target="_new" href="http://hutorny.in.ua/projects/micurpc">µcuRPC Library</a></div>' +
			'<label>References...</label>' +
			'<div><a target="_new" href="https://www.arduino.cc/en/Reference/HomePage">Arduino Language Reference</a></div>'; 
	}
	var left = $one('#left');
	if( left ) 
